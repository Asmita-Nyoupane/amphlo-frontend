...
const Excellence = () => {
    return (
        <div className='flex flex-col lg:flex-row gap-14 w-11/12 mx-auto  text-white px-4 py-20' data-aos="fade-up">
            ...
            <section className="relative w-full lg:w-[50%] h-64 mx-auto text-center grid grid-cols-2 gap-4">
                ...
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <div className="w-full h-[0.5px] bg-slate-50 absolute top-1/2 transform -translate-y-1/2" /> {/* X-axis */}
                    <div className="h-full w-[0.5px] bg-slate-50 absolute left-1/2 transform -translate-x-1/2" /> {/* Y-axis */}
                </div>
                ...
                {statistics.map((stat) => (
                    <div key={stat.title} className="flex flex-col items-center gap-2 p-4">
                        ...
                        <span className="text-xl capitalize font-semibold text-gray-200">{stat.title}</span>
                    </div>
                ))}
            </section>
        </div>
